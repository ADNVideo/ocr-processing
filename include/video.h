...
namespace amu {
    ...
    enum VideoType { VideoType_None, VideoType_Video, VideoType_Repere };
...
//                        "  --start <double>                  start video at that time \n"
...
//                        "  --end-frame <int>                 end video at that frame\n"
            ...
            bool Configure(CommandLine& options, bool failIfNotLoaded = true) {
                ...
                if(numActive > 1) {
                    std::cerr << "ERROR: can only specify one of video or repere-video\n";
                    return false;
                }
